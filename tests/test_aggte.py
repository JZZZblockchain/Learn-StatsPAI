...
def test_uniform_band_strictly_wider_than_pointwise(cs_result):
    es = aggte(cs_result, type="dynamic", cband=True, random_state=0, n_boot=500)
...
def test_balance_e_restricts_cohorts_and_window(cs_result):
    ...
    es_bal = aggte(cs_result, type="dynamic", balance_e=1, random_state=0, n_boot=300)
...
def test_min_max_e_window(cs_result):
    es = aggte(cs_result, type="dynamic", min_e=-2, max_e=3, random_state=0, n_boot=200)
...
def test_dynamic_overall_averages_post_event_only(cs_result):
    ...
    post_mean = es.detail.loc[es.detail["relative_time"] >= 0, "att"].mean()
...
def test_aggte_rejects_non_cs_result():
    ...
    for u in range(60):
        ...
        for t_ in range(1, 9):
            ...
            rows.append(
                {"i": u, "t": t_, "g": g_val, "y": ui + 0.2 * t_ + te + rng.normal()}
            )
