    ...
    write_results(
        ...
        extra={
            ...
            "aggregation_note": (
                ...
                "against R etwfe(cgroup='never') with etwfe's default "
                "cohort + period fixed effects, the design "
                "sp.etwfe(cgroup='nevertreated') fits since 1.30.0 (through "
                "1.29 it absorbed unit effects and the R call set "
                "ivar=countyreal). Stata jwdid is run with ivar(), which "
                "absorbs unit effects nested in the cluster and so leaves "
                "them out of K: its simple-ATT SEs are sqrt((N-17)/(N-20)) "
                "= 1.000605 below ours and R's on both control groups "
                "(observed 6.06e-4), inside the 1e-3 budget; dropping ivar() "
                "reproduces our SEs to 3e-7 but moves jwdid's estat point "
                "estimates by 1e-6, so the do-file keeps ivar(). The "
                "per-cohort never rows come from sp.wooldridge_did, which "
                "does absorb unit effects, and are compared with "
                "etwfe(ivar=countyreal)."
            ),
            "se_convention_note": (
                ...
                "treatment: they follow R etwfe's default design, whose CR1 "
                "factor counts the cohort levels (fixest's default), and "
                "match R to ~1e-6; the 6.06e-4 gap to Stata jwdid is that "
                "K difference, reconstructed exactly above."
            ),
        },
    )
