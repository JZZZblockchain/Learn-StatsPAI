...
def main() -> None:
    ...
    rows = [
        OrigRecord(
            module=MODULE, side="py", statistic="crude_diff",
            estimate=float(crude), se=None, n=n, published=2.54,
            citation="Hernán-Robins, What If §12.2 (crude)",
        ),
        OrigRecord(
            module=MODULE, side="py", statistic="ipw_att",
            estimate=float(res.estimate), se=float(res.se), n=n,
            published=3.4,
    ...
    print(f"[{MODULE}] crude={crude:.4f} (book 2.54)  "
