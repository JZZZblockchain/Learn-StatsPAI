...
>
> *StatsPAI live numbers last measured: 2026-06-21 on macOS arm64 against the local `statspai` 1.19.0 source tree (post CausalPy-inspired contract sync). External ecosystem comparison rows are retained from the 2026-05-03 measurement unless noted otherwise.*
>
...
| ------------------------------------ | ---------- | -----: | ------------: | ---------------------------------- |
| **StatsPAI** `src/statspai/`         | measured   |    683 |   **332,621** | validation-tiered causal inference |
| StatsPAI tests (`tests/`)            | measured   |    865 |       178,021 | —                                  |
