...
- `validated`: known-truth recovery (a closed form or a deterministic DGP with a known parameter) or replication of published numbers, with no same-byte cross-language comparison. Monte Carlo coverage rows and documented convention disclosures are recorded against a function but never earn a tier on their own (`statspai._parity_taxonomy.validation_tier_for`).
