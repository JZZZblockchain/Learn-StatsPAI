...
StatsPAI 正在 JOSS 审稿中。请按你的角色选择路径：

1. **当前 JOSS 审稿** —— 用 [`docs/joss_reviewer_guide.md`](docs/joss_reviewer_guide.md) 安装并冒烟测试，再到 [`docs/joss_validation_dossier.md`](docs/joss_validation_dossier.md) 查看验证证据；设计动机与常见问题（范围、领域现状、build-vs-contribute）见 [`docs/joss_reviewer_qa.md`](docs/joss_reviewer_qa.md)。
2. 运行 [`examples/`](examples/) 下的离线代表性示例（含 `dml_card.py` 的高维 / double-ML 入口示例）。
3. 贡献与支持路径见 [`CONTRIBUTING.md`](CONTRIBUTING.md)、[`SUPPORT.md`](SUPPORT.md) 以及 [`.github/ISSUE_TEMPLATE/`](.github/ISSUE_TEMPLATE/) 的 issue 模板。

示例使用包内置的教学数据集，安装后无需联网。

**复现高维 / double-ML 数值证据**（与该文献最相关的部分）：StatsPAI 的 rigorous-Lasso 移植逐字复现 `hdm` 的三个 vignette 应用 —— Barro–Lee 增长收敛、Acemoglu–Johnson–Robinson 制度 IV、CPS 2012 性别工资差 —— 并提供 `hdm::rlassologitEffect` 的忠实移植；其 double machine learning 估计量与 `DoubleML` 对齐：

```bash
# rigorous-Lasso (hdm) 对齐 —— 无需 R 或额外依赖（参考值以 JSON 提交）
...
# DoubleML 对齐（可选：需要 parity extra = doubleml-for-py）
pip install -e ".[dev,parity]" && pytest tests/external_parity/test_dml_python_parity.py -q
```

完整 `hdm` ↔ StatsPAI 函数映射见 [`docs/guides/rigorous_lasso_hdm.md`](docs/guides/rigorous_lasso_hdm.md)，401(k) 经典复现见 [`docs/guides/case_study_401k.md`](docs/guides/case_study_401k.md)。
...
**StatsPAI 一句话概览**：live agent registry 中有 1,130 个注册函数 · 87 个子模块 · 333k 行核心代码 + 178k 行测试。这四个数字都可以由唯一的生成器 (`python scripts/registry_stats.py`) 现场复算；[`docs/stats.md`](docs/stats.md) 中的按模块拆分表也由同一个脚本回写。23 个方法家族的 API 广度矩阵以及跨生态行数对比，详见 [`docs/stats.md`](docs/stats.md)。这些覆盖数字描述 API 广度，不等同于每个函数都有 R/Stata 数值验证；生产使用请查看 `validation_status`。

**📦 v1.19.0（2026-06-20）— 跨引擎验证、数据 MCP 归一化、社会网络分析**

StatsPAI 1.19.0 新增 `sp.cross_validate`：用 StatsPAI 原生 + pyfixest +
linearmodels + DoubleML + R 的 `fixest`/`did` + Stata 等多个**相互独立**的引擎
估计同一个 OLS/FE/IV/DML/DiD 模型，并给出 AGREE / PARTIAL / DISAGREE 的诚实判定
（OLS/IV/FE 对齐到机器精度 ~1e-15，DiD 复现 Cunningham 跨包实验）。同时新增
`sp.from_worldbank` / `sp.from_fred` / `sp.from_sdmx`，把数据 MCP 已抓取的
World Bank / FRED / SDMX 负载归一化成整洁面板；以及 numpy/scipy 原生、对齐 R
`igraph`/`sna`/`statnet` 与 Stata `nwcommands` 的社会网络分析模块 `sp.network`。
完整发布说明见 [`CHANGELOG.md`](CHANGELOG.md) `[1.19.0]`。
